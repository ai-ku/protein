...
/*****************************************************************************************************************************/
	...
	if ( argc == 1 || (HAVE_PDB == 0 && HAVE_DCD == 0 && HAVE_PSF == 0 && FILTER==NO))
		...
		printf("\n\033[37m\033[1mcarma v.1.1\033[4;37m____________________________________________________________________\033[0m\n");
		...
		printf("    -bend <3i>  : Calculate bending angle between given atoms\n");
		...
		printf("\033[32mcarma -\033[0m\n");
		printf("\tRead [n*n] values from stdin, and make a postscript bitmap.\n");
...
/*****************************************************************************************************************************/
	...
	else if ( strncasecmp( argv[i], "-PD", 3) == 0 )
		{
			if ( HAVE_PSF == NO || HAVE_DCD == NO )
				{
	...
	else if ( strncasecmp( argv[i], "-ST", 3) == 0 )
		{
			if ( HAVE_PDB != NO )
				printf("\033[31m\033[1mFrame specification for a .pdb file ? Ignored.\033[0m\n");
...
*/			
	...
	else if ( strncasecmp( argv[i], "-SU", 3) == 0 )
		{
			ASA = YES;
		}
...
/*****************************************************************************************************************************/
	...
	if ( VERBOSE )
		printf("\033[32m\033[1mAll done.\033[0m\n");
	myexit(0);
...
/*****************************************************************************************************************************/
	...
	{	
		...
		if ( (unsigned int)(read( dcd, (void *)(&dcd_frame[1]), (size_t)(bytes_per_set) )) != bytes_per_set )
			{
				...
				if ( VERBOSE )
				printf("\033[32m\033[1m\nEnd of DCD reached. All done.\033[0m\n");
				myexit(0);
						}
							}
		...
		if ( BENDFIRST != -1 || BENDSECOND != -1 || BENDTHIRD != -1 )
		{
		FILE	*bend;
		...
		if ( ((float)(min2(headerframes, LAST)) - firstframe +1.0)/(float)(STEP) > 100.0 )
			{
	...
	if ( CRYSTALLOGRAPHIC_FRAME )
		{
			...
			write_map( &filename[0], cmap, gridy, gridx, gridz, iuvw, 2, "Produced from carma v.1.1", cell );
		}
...
char 	name[];
{
	...
	for ( i=0 ; i < nof ; i++ )
		...
		if ( strncasecmp( &string[INDEX_1], &ATOMID[natm*5], 5) == 0 || ALL_ATID == YES || (HEAVY==YES && strncasecmp( &string[INDEX_11], "H", 1) != 0) )
			...
			if ( k == ATOM_TYPES && (MASSES == YES || GYRATION == YES))
				{
					if ( !EXTENDED )
						UNKNOWN = YES;
...
*/
	...
	if ( MAKE_3D == YES )
		{
			...
			fprintf(cns, "       2\n");
			fprintf(cns, " REMARKS FILENAME=\"%s\"\n", filename);
			fprintf(cns, " REMARKS 3D PCA-derived landscape, created by Carma v.1.1\n");
			...
			write_map( &filename[0], inter, new_bins, new_bins, new_bins, iuvw, 2, "Produced from carma v.1.1", cell );
	...
	float	var_expl[201];
	FILE	*out;
	float	variances[2000][2];
	float	differences[2000][2];
	...
	if ( MAP_VARIANCE / ( GRID_POINTS * MAP_RMSD * MAP_RMSD) < 0.65 )
		printf("_YELLOWB_Variance explained is low. Consider using a different cluster analysis method.\033[0m\n");
...
int	bins;
{
	...
	{
		printf("\033[31m\033[1m\n\nCluster analysis made a boo boo! Goodbye.\n");
		exit(1);
	}
